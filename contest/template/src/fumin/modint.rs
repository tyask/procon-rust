...
pub trait Modules: Clone + Copy + PartialEq { const MOD: i64; }
...
impl<M: Modules> Modint<M> {
    ...
    pub fn inv(self) -> Modint<M> { self.pow((Self::MOD-2) as i64) }
...
impl<M: Modules> Readable for Modint<M> { type Output = Modint<M>; fn read<R: BufRead, S: Source<R>>(source: &mut S) -> Self::Output { Modint::new(i64::read(source)) } }
impl<M: Modules> Sum      for Modint<M> { fn sum<I: Iterator<Item=Self>>(iter: I) -> Self { iter.fold(Self::from_t(0), |a, b| a + b) } }

impl<M: Modules, N: IntoT<i64>> FromT<N> for Modint<M> { fn from_t(n: N) -> Self { Self::new(n.into_t()) } }
impl<M: Modules, N: IntoT<i64>> From<N>  for Modint<M> { fn from(n: N)   -> Self { Self::new(n.into_t()) } }
...
impl<M: Modules, N: IntoT<i64>> AddAssign<N> for Modint<M> { fn add_assign(&mut self, rhs: N) { *self += Self::new(rhs); } }
impl<M: Modules, N: IntoT<i64>> SubAssign<N> for Modint<M> { fn sub_assign(&mut self, rhs: N) { *self -= Self::new(rhs); } }
impl<M: Modules, N: IntoT<i64>> MulAssign<N> for Modint<M> { fn mul_assign(&mut self, rhs: N) { *self *= Self::new(rhs); } }
impl<M: Modules, N: IntoT<i64>> DivAssign<N> for Modint<M> { fn div_assign(&mut self, rhs: N) { *self /= Self::new(rhs); } }
impl<M: Modules, N: IntoT<i64>> Add<N> for Modint<M> { type Output = Self; fn add(mut self, rhs: N) -> Self::Output { self += rhs; self } }
impl<M: Modules, N: IntoT<i64>> Sub<N> for Modint<M> { type Output = Self; fn sub(mut self, rhs: N) -> Self::Output { self -= rhs; self } }
impl<M: Modules, N: IntoT<i64>> Mul<N> for Modint<M> { type Output = Self; fn mul(mut self, rhs: N) -> Self::Output { self *= rhs; self } }
impl<M: Modules, N: IntoT<i64>> Div<N> for Modint<M> { type Output = Self; fn div(mut self, rhs: N) -> Self::Output { self /= rhs; self } }
