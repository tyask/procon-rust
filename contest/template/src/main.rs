...
impl<N: SimplePrimInt+Display> Display  for Pt<N> { fn fmt(&self, f: &mut Formatter<'_>) -> Result { write!(f, "{} {}", self.x, self.y) } }
impl<N: SimplePrimInt+Display> Fmt      for Pt<N> { fn fmt(&self) -> String { format!("{} {}", self.x, self.y) } }
