...
fn solve() {
    input! {t:us}
    for _ in 0..t {
        input! {n:us,m:us,x:[us;n],y:[us;m]}
        if !x.iter().all_unique() || !y.iter().all_unique() {
            println!("No");
            continue;
        }

        let mut tt = (1..=n*m).cbset();
        let xt = x.iter().cloned().enumerate().sorted_by_key(|t|t.1).cv();
        let yt = y.iter().cloned().enumerate().sorted_by_key(|t|t.1).cv();
        // let xt = x.iter().cloned().enumerate().cv();
        // let yt = y.iter().cloned().enumerate().cv();
        ...
        for (i,j) in iproduct!(0..n,0..m) {
            if a[xt[i].0][yt[j].0] != 0 { continue; }
            let mx = xt[i].1.min(yt[j].1);
            if let Some(&v) = tt.range(..=mx).last() {
                a[xt[i].0][yt[j].0] = v;
                tt.remove(&v);
            } else {
                ok = false;
        ...
        if ok {
            println!("Yes");
            for i in 0..n { vprintsp!(a[i]); }
            // for i in 0..n { assert!((0..m).map(|j|a[i][j]).max().unwrap()==x[i]); }
            // for j in 0..m { assert!((0..n).map(|i|a[i][j]).max().unwrap()==y[j]); }
        } else {
            println!("No");
        }
    }
